...
import { generateMultipleABNs } from '../../utils/abn';

export function AbnGenerator() {
  const [count, setCount] = useState(1);
  const [abns, setAbns] = useState<string[]>([]);
  const { isCopied, copyToClipboard } = useCopyToClipboard();

  const handleGenerate = () => {
    setAbns(generateMultipleABNs(count));
  };

  const handleCopyAll = () => {
    copyToClipboard(abns.join('\n'));
  };

  return (
    <div className="space-y-6 max-w-2xl mx-auto">
      <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
        <h2 className="text-lg font-medium mb-4">Generate Australian Business Numbers</h2>
              ...
              <ul className="space-y-2">
                {abns.map((abn, idx) => (
                  <li key={idx} className="font-mono text-sm flex justify-between items-center p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors">
                    <span>{abn.replace(/(\d{2})(\d{3})(\d{3})(\d{3})/, '$1 $2 $3 $4')}</span>
                    <CopyButton text={abn} />
                  </li>
                ))}
              </ul>
